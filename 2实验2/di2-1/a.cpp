...
int main()
{
     ...
     SqStack S;
SElemType x, e;
     if(InitStack(S)==1)    // 判断顺序表是否创建成功，请填空
{
...
while(1)
	...
	scanf("%d",&a);
		...
		{
			case 1: scanf("%d", &x);
		      if(Push(S,x)==0) printf("Push Error!\n"); // 判断Push是否合法，请填空
		      else printf("The Element %d is Successfully Pushed!\n", x); 
		      break;
	    	case 2: if(Pop(S,e)==0) printf("Pop Error!\n"); // 判断Pop是否合法，请填空
			  else printf("The Element %d is Successfully Poped!\n", e);
		  	  break;
		    case 3: if(GetTop(S,e)==0)printf("Get Top Error!\n"); // 判断Get Top是否合法，请填空
