...
int main()
{
  ...
  SqQueue S;
	QElemType x, e;
  if(InitQueue(S)!=0)    // 判断顺序表是否创建成功，请填空
	{
	...
	printf("1:Enter \n2:Delete \n3:Get the Front \n4:Return the Length of the Queue\n5:Load the Queue\n0:Exit\nPlease choose:\n");
		...
		{
			case 1: scanf("%d", &x);
				  if(EnQueue(S,x)==0) printf("Enter Error!\n"); // 判断入队是否合法，请填空
				  else printf("The Element %d is Successfully Entered!\n", x); 
				  break;
			case 2: if(DeQueue(S,e)==0) printf("Delete Error!\n"); // 判断出队是否合法，请填空
				  else printf("The Element %d is Successfully Deleted!\n", e);
				  break;
			case 3: if(GetHead(S,e)==0)printf("Get Head Error!\n"); // 判断Get Head是否合法，请填空
